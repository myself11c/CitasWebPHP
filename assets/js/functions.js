...
'<button type="button" id="singlebutton" name="singlebutton" onclick="MostrarListaSede(\'new\')" class="btn bg-gradient-info  active me-2"> Siguiente </button>'+
'<button type="button"  onclick="location.reload();" class="btn bg-gradient-warning  active ms-2">Cancelar</button>'+
...
'<!-- Select Basic -->'+
'<label for="tipo">Prestador</label>'+
'<div class="mb-3">'+
...
'<button id="singlebutton" name="singlebutton" onclick="ListDoctors(\'new\')" class="btn bg-gradient-info">Siguiente</button>'+
'<button type="button"  onclick="location.reload();" class="btn w-100 mt-4 mb-0 bg-gradient-warning">Cancelar</button>';
...
swal.fire({
  ...
  showCancelButton: true,
    customClass: {
        confirmButton: 'btn bg-gradient-info btn-lg active me-2',
        cancelButton: 'btn bg-gradient-warning btn-lg active ms-2',
    },
    ...
    $.ajax(
      {
         ...
         success: function (data) {
           ...
           var mensaje = data.Mensaje;
           if (estado =='200'){
             $('#fecha_cita_sios').html('<b>'+data.Cita.Fecha+'</b>');
             $('#sede_sios').html('<b>'+data.Cita.Sede+'</b>');
             $('#prestador_sios').html('<b>'+data.Cita.Prestador+'</b>');
             $('#especilidad_sios').html('<b>'+data.Cita.Especialidad+'</b>');
...
function BuscarPacientePorId(idPaciente) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (estado == "200") {
                $("#tipo_id_sios").html(
                    "<b>" + data.Paciente.TipoIdentificacion + "</b>"
                );
                $("#idenificacion_paciente_sios").html(
                    "<b>" + data.Paciente.NumeroIdentificacion + "</b>"
                );
                $("#nombre_paciente_sios").html(
                    "<b>" +
                    data.Paciente.PrimerNombre +
                    " " +
                    data.Paciente.SegundoNombre +
                    " " +
                    data.Paciente.PrimerApellido +
                    " " +
...
function showUser(cod_pac, type_id)
{
  ...
  var headerHTML = "<div class=\"table-responsive\">" +
      "<table border='1' cellpadding='2' cellspacing='2' class='table align-items-center mb-0'><thead><tr>"+
  ...
  "<th class='text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2'>Primer Nombre</th>" +
  "<th class='text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2'>Segundo Nombre</th>"+
  "<th class='text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2'>Primer Apellido</th>" +
  "<th class='text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2'>Segundo Apellido</th>" +
  "</tr></thead></div>";
  var footerHTML= "</table><br>"+
  '<button type="button" id="singlebutton" name="singlebutton" onclick="MostrarListaSede(\'no\')" class="btn bg-gradient-info  active me-2"> Siguiente</button>'+
      '<button type="button"  onclick="location.reload();" class="btn bg-gradient-warning  active ms-2">Cancelar</button>';
  var objJson = new Object();
    ...
    $.ajax(
     {
        ...
        success: function (data) {
          ...
          var estado= data.Estado;
          if (estado =='200'){
              var trHTML=  '<label>Nombre</label>'+
              '<div className="mb-3">'+
                  '<label>'+data.Paciente.PrimerNombre+' '+data.Paciente.SegundoNombre+' '+data.Paciente.PrimerApellido+' '+data.Paciente.SegundoApellido+' '+'</label>'+
              '</div>';
            ...
            $('#inf_pac').html(trHTML+sios_id_pac);
...
function verificarCampos(camposformulario) {
    let ok = true;
    for (const id of camposformulario) {
        let idElemento = `#${id}`;
...
function valUser(){
    ...
    let camposformulario = ["type_id", "id_pac", "select_tipo", "sFechaDeseada"];
    let ok = verificarCampos(camposformulario);

    $("#messages").html(loader_div);

    if (ok == true) {
        TipoCita = $("#select_tipo option:selected").text();
        ...
        swal.fire(
            "mensaje",
            "los campos marcados en rojo son obligatoros",
            "error"
        );
    }
}
          ...
          if (estado =='200'){
            ...
            for (var i = 0, len = data.ListaTurnos.length; i < len; i++) {
                if(fecha_hoy!=data.ListaTurnos[i].sFecha){
                    ...
                    selectPrestador+='<option value=\"'+(data.ListaTurnos[i].IdPrestador)+'*'+data.ListaTurnos[i].Fecha+'*'+data.ListaTurnos[i].Prestador+'*'+data.ListaTurnos[i].IdSede+'\">'+data.ListaTurnos[i].Prestador+'    -    '+data.ListaTurnos[i].Fecha+' - '+data.ListaTurnos[i].CuposDisponibles+'</option>';
                    //}
                }
              console.log(data.ListaTurnos[i].IdPrestador+' : '+data.ListaTurnos[i].Prestador+' : '+data.ListaTurnos[i].Fecha)+' : '+data.ListaTurnos[i].CuposDisponibles;
              ...
              var dcancelada=data.ListaCitas[i].Cancelada;
                ...
                if(cancelar==true && dcancelada== 'NO'){
                 ...
                 htmlCard= '<div class="col-md-12 mb-4">'+
                    ...
                    data.ListaCitas[i].Fecha+
                    '</p>'+
                '</div>'+
                '<div class="text-center">'+
                     '<span class="badge rounded-pill bg-light text-dark">Sede</span>'+
                     '<p class="font-weight-bold mt-2">'+
                     data.ListaCitas[i].Sede+
                     '</p>'+
                '</div>'+
                 '<div class="text-center">'+
                    '<span class="badge rounded-pill bg-light text-dark">Prestador</span>'+
                          '<p class="font-weight-bold mt-2">'+
                           data.ListaCitas[i].Prestador+
                          '</p>'+
                 '</div>'+
                 '<div class="text-center">'+
                     '<span class="badge rounded-pill bg-light text-dark">Especialidad</span>'+
                        '<p class="font-weight-bold mt-2">'+
                            data.ListaCitas[i].Especialidad+
                        '</p>'+
                 '</div>'+
                     '<div class="text-center">'+
                     '<span class="badge rounded-pill bg-light text-dark">Cancelada</span>'+
                     '<p class="font-weight-bold mt-2">'+
                     data.ListaCitas[i].Cancelada+
                     '</p>'+
                     '</div>'+
                     '<div class="text-center">'+
                     '<span class="badge rounded-pill bg-light text-dark">Atendido</span>'+
                     '<p class="font-weight-bold mt-2">'+
                     data.ListaCitas[i].Atendido+
                     '</p>'+
                     '</div>'+
                '<div class="d-flex justify-content-lg-start justify-content-center p-2">'+
                '<div class="bg-gradient-secondary shadow text-center">'+
                '</div>'+
                '<div>'+
                        htmlCancelar+
                '</a>'+
  ...
  $.ajax(
    {
       ...
       success: function (data) {
        ...
        console.log('Object JSON'+JSON.stringify(objJson) );
         ...
         if (estado =='200'){
          cod_pac=data.Paciente.NumeroIdentificacion;
          nombre_paciente_sios  = data.Paciente.PrimerNombre+' '+data.Paciente.SegundoNombre+
          ' '+data.Paciente.PrimerApellido+' '+data.Paciente.SegundoApellido;
           IdPacienteSios=data.Paciente.IdPaciente;
           console.log('ID Paciente SIOS:'+data.Paciente.IdPaciente);
           ...
           verHistorialCitas(IdPacienteSios);
           console.log('ID Paciente SIOS:'+IdPacienteSios);
         }
         if (estado =='404'){
           //Error -- 
            ...
            swal.fire({
              title: 'Atencion!',
              html: 'Los datos ingresados no corresponden a ningun usuario existente.',
              type: 'error',
              showCancelButton: false,
              confirmButtonColor: '#3085d6',
              confirmButtonText: 'Aceptar',  
